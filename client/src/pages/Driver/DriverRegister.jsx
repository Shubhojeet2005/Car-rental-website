...
const DriverRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.append('car_license_no', formData.car_license_no);
      formDataToSend.append('car_name', formData.car_name);
      formDataToSend.append('password', formData.password);
      if (formData.doc_upload) {
        formDataToSend.append('doc_upload', formData.doc_upload);
      }
      if (formData.car_picture) {
        formDataToSend.append('car_picture', formData.car_picture);
      }

      const res = await fetch(`${API_URL}/user/register_driver`, {
        method: 'POST',
        body: formDataToSend,
      });
      ...
      >
        <div className="bg-slate-900/40 backdrop-blur-3xl border border-white/10 p-10 rounded-[2.5rem] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.8)] ring-1 ring-white/10">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-slate-400 mt-3 font-medium">Create your driver account to get started</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-[0.1em] text-slate-500 ml-1">Car Picture</label>
                <input
                  type="file"
                  name="car_picture"
                  accept="image/*"
                  onChange={handleChange}
                  className="w-full bg-white/[0.03] border border-white/10 text-slate-400 text-sm rounded-2xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 focus:bg-white/[0.08] outline-none transition-all px-5 py-4 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-indigo-600 file:text-white hover:file:bg-indigo-500 cursor-pointer"
                  required
                />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-[0.1em] text-slate-500 ml-1">Driver Documents</label>
                <input
                  type="file"
                  name="doc_upload"
                  onChange={handleChange}
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-[0.1em] text-slate-500 ml-1">Phone Number</label>
              <input
                type="tel"
                name="phone"
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-[0.1em] text-slate-500 ml-1">Driver License No.</label>
                <input
                  type="text"
                  name="car_license_no"
                  value={formData.car_license_no}
                  onChange={handleChange}
                  className="w-full bg-white/[0.03] border border-white/10 text-white text-sm rounded-2xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 focus:bg-white/[0.08] outline-none transition-all px-5 py-4 placeholder:text-slate-600"
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-[0.1em] text-slate-500 ml-1">Car Name</label>
                <input
                  type="text"
                  name="car_name"
